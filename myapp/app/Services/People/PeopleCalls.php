...
class PeopleCalls
{
    public function people_add($json)
    {
        ...
        $organization_uid = DB::table('organization')
            ->where('name', DBValues::DB_OPERATOR_EQUAL_TO, $people_json->{'organisation'})
            ->select('uid')
            ->get();
    ...
    public function people_populate($uid)
    {
        $data_response = DB::table(DBValues::DB_TABLE_NAME_PEOPLE)
            ->join('organization', 'organization.uid', DBValues::DB_OPERATOR_EQUAL_TO, DBValues::DB_TABLE_NAME_PEOPLE . DBValues::DB_OPERATOR_DOT . DBValues::DB_TABLE_PEOPLE_ORGANISATION_ID)
    ...
    public function organization_names_from_people_table(Request $request)
    {
        ...
        return response()->json($json_organization_names)->header("Access-Control-Allow-Origin", "*");
    }
}
